...
// Set nilai default untuk $orderid
$orderidd = null;

// Mencari cart yang sesuai dengan user id dan status 'Cart'
$caricart = mysqli_query($conn, "SELECT * FROM cart WHERE userid='$uid' AND status='Cart'");
$fetc = mysqli_fetch_array($caricart);

// Memeriksa apakah query berhasil mengembalikan hasil
if ($fetc) {
	$orderidd = $fetc['orderid'];
	// Menghitung jumlah transaksi di detailorder
	$itungtrans = mysqli_query($conn, "SELECT COUNT(detailid) AS jumlahtrans FROM detailorder WHERE orderid='$orderidd'");
	$itungtrans2 = mysqli_fetch_assoc($itungtrans);
	$itungtrans3 = $itungtrans2['jumlahtrans'];
} else {
	// Jika tidak ada cart ditemukan, set $itungtrans3 jadi 0
	$itungtrans3 = 0;
}
...
SELECT p.idproduk, p.namaproduk, p.gambar, vp.harga AS hargaafter, d.qty
FROM detailorder d
JOIN produk p ON d.idproduk = p.idproduk
LEFT JOIN varian_produk vp ON vp.idvarian = d.idvarian
...
");
		...
		<div class="checkout-left">
			<div class="checkout-right-basket">
				...
				<a href="checkout.php"><span class="glyphicon glyphicon-menu-right" aria-hidden="true"></span>Beli dan Bayar Sekarang</a>
				<a href="vendor/midtrans/midtrans-php/examples/snap/checkout-process-simple-version.php"><span class="glyphicon glyphicon-menu-right" aria-hidden="true"></span> Barang Diambil</a>
			</div>
			<div class="clearfix"> </div>
		</div>
	</div>
