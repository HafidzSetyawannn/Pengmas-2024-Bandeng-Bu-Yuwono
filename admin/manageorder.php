...
<head>
    ...
    <title>Kelola Pesanan - Bandeng Bu Yuwono</title>
...
<body>
    ...
    <div class="page-container">
                            ...
                            <div class="card-body">
                                ...
                                <div class="data-tables datatable-dark">
                                    <table id="dataTable3" class="display" style="width:100%">
                                        ...
                                        <tbody>
                                            <?php
                                            $brgs = mysqli_query($conn, "SELECT c.orderid, c.tglorder, c.alamat, c.status, l.namalengkap FROM cart c JOIN login l ON c.userid = l.userid WHERE c.status != 'Cart' AND c.status != 'Selesai' ORDER BY c.idcart ASC");
                                            $no = 1;
                                            while ($p = mysqli_fetch_array($brgs)) {
                                                $orderids = $p['orderid'];
                                                $ongkir = 10000; // Ongkos kirim tetap sebesar Rp10.000
                                            ?>
                                                <tr>
                                                    <td><?php echo $no++ ?></td>
        ...
        SELECT SUM(d.qty * vp.harga) AS count 
        FROM detailorder d
        JOIN varian_produk vp ON d.idvarian = vp.idvarian
        WHERE d.orderid = '$orderids'
    ");
                                                            $row1 = mysqli_fetch_assoc($result1);
                                                            $total_harga = $row1['count'] + $ongkir; // Tambahkan ongkir ke total harga
                                                            echo number_format($total_harga);
                                                            ?></td>
                                                    <td><?php echo $p['status']; ?></td>
                                                    <td><?php echo $p['alamat']; ?></td>
                                                    <td>
                                                        <a href="edit_orderOnline.php?orderid=<?php echo $p['orderid'] ?>" class="btn btn-warning btn-sm">Edit</a>
                                                        <a href="delete_orderOnline.php?orderid=<?php echo $p['orderid'] ?>" onclick="return confirm('Apakah Anda yakin ingin menghapus pesanan ini?')" class="btn btn-danger btn-sm">Hapus</a>
                                                    </td>
                                                </tr>
                                            <?php
                                            }
