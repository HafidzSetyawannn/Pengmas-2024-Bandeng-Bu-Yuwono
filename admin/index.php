...
// Hitung pendapatan dari detailorder (produk yang dibeli online)
$itungincome_online = mysqli_query($conn, "
    SELECT SUM(d.qty * p.hargaafter) AS jumlahincome 
    FROM detailorder d 
    JOIN produk p ON d.idproduk = p.idproduk 
    WHERE MONTH(d.tanggal_transaksi) = MONTH(CURRENT_DATE())
    AND YEAR(d.tanggal_transaksi) = YEAR(CURRENT_DATE())
");
$itungincome_online2 = mysqli_fetch_assoc($itungincome_online);
$income_online = $itungincome_online2 ? $itungincome_online2['jumlahincome'] : 0;

// Hitung pendapatan dari offline_orders (produk yang dibeli secara offline)
$itungincome_offline = mysqli_query($conn, "
    SELECT SUM(total_price) AS jumlahincome 
    FROM offline_orders
    WHERE MONTH(order_date) = MONTH(CURRENT_DATE())
    AND YEAR(order_date) = YEAR(CURRENT_DATE())
");
$itungincome_offline2 = mysqli_fetch_assoc($itungincome_offline);
$income_offline = $itungincome_offline2 ? $itungincome_offline2['jumlahincome'] : 0;

// Jumlahkan pendapatan dari online dan offline
$total_income = $income_online + $income_offline;

// Jika ada pendapatan, tampilkan, jika tidak, set ke 0
if ($total_income) {
...
<head>
    ...
    <title>Admin Panel - Bandeng Bu Yuwono</title>
            ...
            <div class="main-content-inner">
                ...
                <div class="sales-report-area mt-5 mb-5">
                    <div class="row">
                        ...
                        <div class="col-md-3">
                            <div class="single-report mb-xs-30">
                                <div class="s-report-inner pr--20 pt--30 mb-3">
                                    ...
                                    <div class="s-report-title d-flex justify-content-between">
                                        <h3 class="header-title mb-0">Konfirmasi</h3>
                                    </div>
                                    <div class="d-flex justify-content-between pb-2">
                        ...
                        // Query to calculate total income from online orders (detailorder)
                        $query_income = "SELECT SUM(d.qty * vp.harga) AS total_income
                 FROM detailorder d
                 JOIN varian_produk vp ON d.idvarian = vp.idvarian";
                        ...
                        // Get the total income from online orders
                        $total_income_online = $row_income['total_income'] ?? 0;

                        // Query to calculate total income from offline orders (offline_orders)
                        $query_offline_income = "SELECT SUM(total_price) AS total_offline_income
                         FROM offline_orders";

                        // Execute the query for offline orders
                        $result_offline_income = mysqli_query($conn, $query_offline_income);

                        // Fetch the result
                        $row_offline_income = mysqli_fetch_assoc($result_offline_income);

                        // Get the total income from offline orders
                        $total_income_offline = $row_offline_income['total_offline_income'] ?? 0;

                        // Combine both incomes
                        $total_income = $total_income_online + $total_income_offline;
                        ?>
                            ...
                            <div class="single-report">
                                <div class="s-report-inner pr--20 pt--30 mb-3">
                                    ...
                                    <div class="d-flex justify-content-between pb-2">
                                        <h3>
                                            <?php
                                            // Format total income with thousands separator and currency format (Rupiah)
                                            echo "Rp. " . number_format($total_income, 0, ',', '.');
                                            ?>
                                        </h3>
                                    </div>
